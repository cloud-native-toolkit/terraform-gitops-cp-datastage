...
CSV=""
count=0
csvstr="ibm-cpd-datastage-operator."
while [ true ]; do
  sleep 60
...
echo "DataStage Operator is READY"
sleep 60
INSTANCE_STATUS=""

while [ true ]; do
  INSTANCE_STATUS=$(kubectl get DataStage datastage -n "${CPD_NAMESPACE}" -o jsonpath='{.status.dsStatus} {"\n"}')
...
echo "DataStage DS/"${INSTANCE_NAME}" is "${INSTANCE_STATUS}""
